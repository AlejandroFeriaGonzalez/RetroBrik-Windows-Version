...
class Parser:
    ...
    def parsear_shape(self):
        ...
        estados = []
        while (
            self.posicion < len(self.tokens) and self.tokens[self.posicion] == "STATE"
        ):
            self.consumir("STATE")
            self.consumir()
            self.consumir(":")
            matriz = []
            while (
                self.posicion < len(self.tokens) and self.tokens[self.posicion] == "["
            ):
                fila = []
                self.consumir("[")
        ...
        while self.posicion < len(self.tokens) and self.tokens[self.posicion] != "END":
            ...
            if self.posicion < len(self.tokens) and self.tokens[self.posicion] == "AT":
                ...
                if self.tokens[self.posicion] == "RANDOM":
                    ...
                    self.consumir(")")
                    params.append([x, y])
            elif self.posicion < len(self.tokens) and self.tokens[
                self.posicion
            ] not in [
                "END",
                "ON",
